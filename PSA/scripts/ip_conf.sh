...
# ip_conf.sh
#
# This script is called by the PSA API when the PSA should be configured with IP
# address.
#
# NOTE: This script is called right after init.sh script at the start-up of a
# PSA.
#
# !!!
# This should have the base setup for IP. init.sh should not change these
# values, since it will overwrite these values at the moment.
# !!!
#
# --> (We can change the logic to call this after init.sh always?)
#

# Just a place-holder as Bro PSA does not use this script..
#if [ -z "$PSA_HOME" ]; then
...
# Note that now we just replace any existing conf, since this should be the only
# DNS for the PSA.
SEARCH='nameserver '$3
